...
typedef struct clientContext_struct
{
	...
	uint8_t *publicKeySignature; // Voir pour le stockage des clefs
...
typedef struct bzrtpHelloMessage_struct {
	...
	//char * name[MAX_NAME_LENGTH]; /**< the name of the ZRTP end point */
...
typedef struct bzrtpCommitMessage_struct {
	...
	uint8_t publicKey[PQCLEAN_KYBER1024_CLEAN_CRYPTO_PUBLICKEYBYTES]; /**< Public Key for the shared secret based on Kyber*/
...
typedef struct bzrtpDHPartMessage_struct {
	...
	uint8_t MAC[8]; /**< HMAC over the whole message, keyed by the hash image H1 (64 bits)*/
	uint8_t cipherText[PQCLEAN_KYBER1024_CLEAN_CRYPTO_CIPHERTEXTBYTES]; /**< Cipher Text for decrypted the shared secret based on Kyber*/
