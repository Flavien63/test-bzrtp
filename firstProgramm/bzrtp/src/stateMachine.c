...
int state_keyAgreement_initiatorSendingDHPart2(bzrtpEvent_t event) {
	...
	if (event.eventType == BZRTP_EVENT_MESSAGE) {
		...
		if (zrtpPacket->messageType == MSGTYPE_CONFIRM1) {
			...
			/* process the signature and copy that */
			for (int i = 0; i < PQCLEAN_DILITHIUM5_CLEAN_CRYPTO_BYTES; i++)
			{
				signature[i] = confirm1Packet->signatureBlock[i];
			}
			...
			{
				printf("Signature is ok for Confirm1\n");
			}

			/* free the sas and the signature */
			free(sas);
			free(signature);
...
int state_confirmation_responderSendingConfirm1(bzrtpEvent_t event) {
	...
	if (event.eventType == BZRTP_EVENT_INIT) {
		...
		bzrtpConfirmMessage_t *confirm1Message = confirm1Packet->messageData;
		...
		/* malloc the signature block to inser the signature previously calculated */
		...
		for (int i = 0; i < PQCLEAN_DILITHIUM5_CLEAN_CRYPTO_BYTES; i++)
		{
			confirm1Message->signatureBlock[i] = signature[i];
		}
	...
	if (event.eventType == BZRTP_EVENT_MESSAGE) {
		...
		if (zrtpPacket->messageType == MSGTYPE_CONFIRM2) {
			...
			{
				printf("Signature is ok for Confirm2\n");
			}
...
/**
 * @brief This function is called after DHPart2 is received by responder or confirm1 by initiator
 * Keys computed are: SAS(if mode is not multistream).
 *
