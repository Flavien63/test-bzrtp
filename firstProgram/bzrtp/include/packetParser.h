...
typedef struct clientContext_struct
{
	...
	uint8_t *publicKeySignature;
	uint8_t *privateKeySignature;
	uint8_t *peerPublicKeySignature;
	uint8_t *publicKeySharedSecret;
	uint8_t *privateKeySharedSecret;
	uint8_t *peerPublicKeySharedSecret;
	uint8_t *cipherText;
	uint8_t *secretShared;
...
typedef struct bzrtpCommitMessage_struct {
	...
	uint8_t publicKey[PQCLEAN_KYBER768_CLEAN_CRYPTO_PUBLICKEYBYTES]; /**< Public Key for the shared secret based on Kyber*/
...
typedef struct bzrtpDHPartMessage_struct {
	...
	uint8_t MAC[8]; /**< HMAC over the whole message, keyed by the hash image H1 (64 bits)*/
	uint8_t cipherText[PQCLEAN_KYBER768_CLEAN_CRYPTO_CIPHERTEXTBYTES]; /**< Cipher Text for decrypted the shared secret based on Kyber*/
